 ...
 * ==========
 */
var entertainment = new keystone.List('Entertainment', {
	map: { name: 'title' },
...
entertainment.add({
	...
	image: { type: Types.LocalFile ,dest: __dirname+'/../public/advert/', label: '图像',allowedTypes :['image/jpeg'],filename: function(item, name){
		return item.property+"/entertainment/"+item.title+"_"+item.telephone+"_"+item.cellphone+"_"+item.contact+"_"+item._.publishedDate.format('YYYY-MM-DD')+"_"+name;
...
entertainment.defaultColumns = 'title, address|20%, detail|20%';
